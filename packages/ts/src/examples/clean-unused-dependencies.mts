...
for await (const { filename, sourceFile, declarations } of walkModuleDependencyImports(
  ...
  if (
    !declarations.isMetaImport &&
    !declarations.isExportedImport &&
    (declarations.namespaceImport ||
      declarations.defaultImport ||
      declarations.namedImports.length > 0)
  ) {
    let importIdentifiers = [
      ...(declarations.namespaceImport && declarations.namespaceImport !== 'React'
        ? [declarations.namespaceImport]
        : []),
      ...(declarations.defaultImport && declarations.defaultImport !== 'React'
        ? [declarations.defaultImport]
        : []),
    ...
    for (const id of sourceFile.getDescendantsOfKind(SyntaxKind.Identifier)) {
      ...
      if (
        !ts.isImportClause(parent.compilerNode) &&
        !ts.isImportSpecifier(parent.compilerNode) &&
        !ts.isNamespaceImport(parent.compilerNode) &&
        (startsWithUppercaseLetter(id.getText()) ||
          (!ts.isJsxOpeningElement(parent.compilerNode) &&
            !ts.isJsxClosingElement(parent.compilerNode) &&
            !ts.isJsxSelfClosingElement(parent.compilerNode)))
      ) {
    ...
    const declaration = NNU(
      sourceFile.getImportDeclaration(
        decl => decl.getModuleSpecifier().getLiteralValue() === declarations.moduleSpecifier,
      ),
      `No declaration found matching '${declarations.moduleSpecifier}' in '${filename}`,
    )
    const defaultImport = declaration.getDefaultImport()
    if (defaultImport && importIdentifiers.includes(defaultImport.getText())) {
      console.warn(
        `  Unused import: '${defaultImport.getText()}' from '${declaration.getModuleSpecifier().getLiteralValue()}'`,
      )
    ...
    if (namespaceImport && importIdentifiers.includes(namespaceImport.getText())) {
      console.warn(
        `  Unused import: '${namespaceImport.getText()}' from '${declaration.getModuleSpecifier().getLiteralValue()}'`,
      )
    ...
    for (const namedImport of declaration.getNamedImports()) {
      if (importIdentifiers.includes(namedImport.getText())) {
        console.warn(
          `  Unused import: '${namedImport.getText()}' from '${declaration.getModuleSpecifier().getLiteralValue()}'`,
        )
        namedImport.remove()
      }
    }
