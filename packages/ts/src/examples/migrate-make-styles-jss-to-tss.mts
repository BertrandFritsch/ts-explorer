...
import { asserts, initializeRootDirectory, NNU, walkModuleDependencyImports, } from '@bertrand.fritsch/ts-explorer-lib'
...
program
  .name('migrate-make-styles-jss-to-tss')
...
async function migrateJSS2TSS() {
  ...
  for await (const { filename, sourceFile, namedImport, importDeclaration } of walkModuleImports(
    items,
  )) {
    let updated = false
    ...
    for (const callExpression of sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)) {
      ...
      if (Node.isIdentifier(callee) && callee.getText() === namedImport) {
        ...
        asserts(
          makeStylesArgs.length === 1,
          `Expected the call of '${namedImport}' to have one argument only!`,
        )
        ...
        if (Node.isArrowFunction(makeStylesArg)) {
          if (makeStylesArg.getParameters().length === 0) {
            ...
            asserts(
              ...
              `Expected the second child of the arrow function to be a parenthesized expression! Found: '${body.getKindName()}'.`,
            )

            const parenthesizedExpressionChild = body.getExpression()
            asserts(
              Node.isObjectLiteralExpression(parenthesizedExpressionChild),
              `Expected the parenthesized expression to be an object literal expression! Found: '${body.getKindName()}'.`,
            )
        ...
        asserts(
          Node.isExpression(makeStylesArg),
          `Expect to be an expression! Found: '${makeStylesArg.getKindName()}'.`,
        )
        callExpression
          .replaceWithText(
            ts
              ...
              .printNode(
                ...
                ts.factory.createCallExpression(callExpression.compilerNode, undefined, [
                  makeStylesArg.compilerNode,
                ]),
        ...
        if (Node.isExportAssignment(parentExpression)) {
          ...
          asserts(
            Node.isVariableDeclaration(parentExpression),
            `Expected the parent of '${namedImport}' to be a variable declaration! Found: '${parentExpression?.getKindName()}'.`,
          )

          const variableIdentifier = parentExpression.getFirstChild()
          asserts(
            Node.isIdentifier(variableIdentifier),
            `Expected the variable declaration to be an identifier! Found: '${variableIdentifier?.getKindName()}'.`,
          )
          ...
          if (
            parentExpression
              ...
              ?.getFirstChild()
              ?.getKind() === SyntaxKind.ExportKeyword
          ) {
...
async function updateImportTSSClauses(item: string) {
  for await (const { filename, sourceFile, namedImport } of walkModuleImports([
    parseModuleItem(item),
  ])) {
  ...
  for (const callExpression of sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)) {
    ...
    if (Node.isIdentifier(callee) && callee.getText() === item) {
      ...
      asserts(
        ...
        `Expected the parent of '${item}' to be a variable declaration! Found: '${variableDeclaration?.getKindName()}'.`,
      )
            ...
            ts.factory.createVariableDeclaration(
              ts.factory.createObjectBindingPattern([
                ts.factory.createBindingElement(
                  undefined,
                  undefined,
                  variableIdentifier.compilerNode,
                  undefined,
                ),
              ]),
              undefined,
...
async function* walkModuleImports(items: Item[]) {
  for await (const {
    ...
    declarations,
    importDeclaration,
  } of walkModuleDependencyImports(sourceFiles, {
    skipAddingFilesFromTsConfig: false,
    walkThroughImports: false,
  })) {
    if (!items.some(item => !item.isExternal && item.moduleSpecifier === filename)) {
