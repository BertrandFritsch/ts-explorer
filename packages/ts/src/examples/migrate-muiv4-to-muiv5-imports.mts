...
program
  .name('migrate-muiv4-to-muiv5-imports')
...
function collectImportsToMigrate(
  v4Package: string,
  v5Package: string,
  ...
  const match = declarations.moduleSpecifier.match(
    new RegExp(`^${v4Package}(?:/(?<component>.*?))?(?:/|$)`),
  )
  if (match) {
    ...
    asserts(
      ...
      `Import declaration not found for '${declarations.moduleSpecifier}'`,
    )
    const muiFilename =
      muiImports.get(filename) ||
      NNU(muiImports.set(filename, { sourceFile, imports: new Map() }).get(filename))

    if (match.groups?.component) {
      const muiImport =
        muiFilename.imports.get(match.groups.component) ||
        NNU(
          muiFilename.imports
            .set(match.groups.component, { v5Package, defaultImportName: null, otherImports: [] })
            .get(match.groups.component),
        )
      muiImport.defaultImportName =
        (muiImport.defaultImportName || declarations.defaultImport) ?? null
      ...
      for (const namedImport of declarations.namedImports) {
        if (startsWithUppercaseLetter(namedImport.name)) {
          ...
          const muiImport =
            muiFilename.imports.get(component) ||
            NNU(
              muiFilename.imports
                .set(component, { v5Package, defaultImportName: null, otherImports: [] })
                .get(component),
            )
          ...
          importDeclaration
            .getNamedImports()
            .find(n => n.getName() === namedImport.name)
            ?.remove()
