...
program.name('remove-unused-styles').description('Remove unused styles').version('0.0.1')
...
async function removeUnusedStyles() {
  for await (const { filename, sourceFile } of walkModuleDependencyImports(program.args, {
    ...
    const decl = sourceFile.getImportDeclaration(
      i =>
        i.getModuleSpecifierValue() === 'designSystem/mui/makeStyles' &&
        i.getNamedImports().some(n => n.getText() === 'makeStyles'),
    )
    if (decl === undefined) {
      continue
    }
    ...
    sourceFile.forEachDescendant(
      node => {
        if (
          ...
          node.getText() === 'makeStyles' &&
          Node.isCallExpression(node.getParent())
        ) {
          const makeStylesCall = node.getParent()?.getParent()
          asserts(
            Node.isCallExpression(makeStylesCall),
            `Expected node to be a CallExpression, but was ${node.getKindName()}`,
          )

          const objectExpr = makeStylesCall
            .getChildAtIndex(2)
            .getFirstDescendant((node): node is ObjectLiteralExpression =>
              Node.isObjectLiteralExpression(node),
            )
          asserts(
            objectExpr !== undefined,
            `Expected node to be an ObjectLiteralExpression, but was not found!`,
          )
          ...
          for (const property of objectExpr.getProperties()) {
            asserts(
              ...
              `Expected node to be a PropertyAssignment, but was ${node.getKindName()}`,
            )
          ...
          if (makeStylesCall.getFirstAncestor(node => Node.isExportAssignment(node))) {
            ...
            const variableDeclaration = makeStylesCall.getFirstAncestor(node =>
              Node.isVariableDeclaration(node),
            )
            asserts(
              Node.isVariableDeclaration(variableDeclaration),
              `Expected node to be a VariableDeclaration, but was not found!`,
            )

            const variableName = variableDeclaration.getName()

            const variableStatement = variableDeclaration.getFirstAncestor(node =>
              Node.isVariableStatement(node),
            )
            asserts(
              Node.isVariableStatement(variableStatement),
              `Expected node to be a VariableStatement, but was not found!`,
            )
            ...
            if (variableStatement.isExported()) {
              ...
              asserts(
                ...
                `Expected node to be an Identifier, but was ${nameNode.getKindName()}`,
              )
...
function detectUsedStylesInOtherSourceFiles(
  sourceFile: SourceFile,
  definedClasses: Map<string, PropertyAssignment>,
  exportName: string,
) {
  for (const node of sourceFile.getReferencingNodesInOtherSourceFiles()) {
    asserts(
      Node.isImportDeclaration(node),
      `Expected node to be an ImportDeclaration, but was ${node.getKindName()}`,
    )
...
function detectUsedFilesInSourceFile(
  useStyles: Identifier,
  definedClasses: Map<string, PropertyAssignment>,
) {
  for (const useStylesReference of useStyles
    .findReferencesAsNodes()
    .filter(node => Node.isCallExpression(node.getParent()))) {
    asserts(
      Node.isIdentifier(useStylesReference),
      `Expected node to be an Identifier, but was ${useStylesReference.getKindName()}`,
    )
    const useStylesParent = useStylesReference.getParent()

    asserts(
      Node.isCallExpression(useStylesParent),
      `Expected node to be a CallExpression, but was ${useStylesParent?.getKindName()}`,
    )

    const useStylesVariables = useStylesParent.getParent()
    asserts(
      Node.isVariableDeclaration(useStylesVariables),
      `Expected node to be a VariableDeclaration, but was ${useStylesVariables?.getKindName()}`,
    )
    ...
    const classes = objectBindingPattern
      .getElements()
      .find(
        e =>
          e.getPropertyNameNode()?.getText() === 'classes' ||
          e.getNameNode().getText() === 'classes',
      )
...
function findReferencesAsNodes(node: BindingElement) {
  ...
  function* getReferencingNodes() {
    ...
    for (let i = 1; i < references.length; i++) {
      ...
      if (!reference.isDefinition()) yield reference.getNode()
    }
  }
}
