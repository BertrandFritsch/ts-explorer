...
program
  .name('get-item-dependency-graph')
...
async function getModuleDependencies() {
  ...
  const highlightPaths =
    program.opts().highlightPathsTo &&
    program.opts<Record<string, string[]>>().highlightPathsTo.map(item => parseModuleItem(item))
...
function handlePath(
  ...
  if (
    ...
    appendStack(
      ...
      [
        ...stack,
        item.isExternal ? item.namedImport || item.moduleSpecifier : declarations.resolvedFileName,
      ],
      highlight,
    )
  } else {
  ...
  for (const element of stack) {
    if (highlight || !elements.has(element)) {
      elements.set(element, {
        data: {
          id: element,
          /*parent, */ name: program.opts().keepFullPath
            ? element
            : extractFilenameUserName(element),
          highlight,
        },
      })
    }
    ...
    if (parent) {
      ...
      if (highlight || !elements.has(edgeId)) {
        elements.set(edgeId, {
          data: { source: parent, target: element, pathTarget: stack[stack.length - 1], highlight },
        })
      }
    }
