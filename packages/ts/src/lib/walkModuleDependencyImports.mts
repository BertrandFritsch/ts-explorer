...
export function walkModuleDependencyImports(filenames: string | string[], options?: Partial<WalkModuleDependencyImportOptions>): AsyncGenerator<DependencyGraphItem, void, unknown>;
export function walkModuleDependencyImports(filenames: string | string[], options?: Partial<WalkModuleDependencyGraphOptions>): AsyncGenerator<Omit<DependencyGraphItem, 'declarations' | 'importDeclaration' | 'exportDeclaration'>, void, unknown>;
export async function* walkModuleDependencyImports(filenames: string | string[], { walkThroughImports = true, skipFileDependencyResolution = true, skipAddingFilesFromTsConfig = true, yieldSourceFileOnlyOnce = false }: Partial<WalkModuleDependencyImportOptions & WalkModuleDependencyGraphOptions> = {}) {
  ...
  for (const filename of Array.isArray(filenames) ? filenames : [ filenames ]) {
    ...
    yield* (async function* resolveImports(module: ResolvedModuleFull, depth: number): AsyncGenerator<Omit<DependencyGraphItem, 'declarations'> & { declarations?: DependencyGraphImport }> {
      ...
      asserts(sourceFile !== undefined, `The source file could not be added: '${ getRelativePath(module.resolvedFileName) }'!`);
