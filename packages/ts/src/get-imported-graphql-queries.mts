...
function locateCallExpressionQuery(callExpression: CallExpression, namedImport: string, argumentIndex: number, queries: QueryItem[], item: Item, filename: string) {
  ...
  if (Node.isIdentifier(arg)) {
    ...
    asserts(Node.isIdentifier(property), `Expected the property access reference being an identifier!`);
...
function locateIdentifierQuery(arg: Identifier, namedImport: string, queries: QueryItem[], item: Item, filename: string) {
  asserts(Node.isIdentifier(arg), `Expect the parameter of '${ namedImport }' to be a primary expression!`);

  const argDefinitions = arg.getDefinitionNodes();
  asserts(argDefinitions.length === 1, `More than one definition found for parameter of '${ namedImport }'`);
  ...
  if (Node.isVariableDeclaration(argDefinition)) {
    ...
    if (initializer && ts.isTaggedTemplateExpression(initializer.compilerNode)) {
      asserts(initializer.compilerNode.tag.getText() === 'gql', `Expected the tagged template expression being a call of 'gql', found '${ initializer.compilerNode.tag }' instead!`);
      queries.push({ queryCaller: stringifyModuleItem(item), filename, queryLocation: getRelativePath(initializer.getSourceFile().getFilePath()), namedImport: arg.compilerNode.getText() });
    }
    else {
      console.error(`No initializer or no tagged expression found for '${ argDefinition.getText() }' in '${ argDefinition.getSourceFile().getFilePath() }'`);
    }
  }
      ...
      if (Node.isVariableDeclaration(functionParent)) {
        ...
        for (const ref of variableName.findReferencesAsNodes()) {
          ...
          if (Node.isCallExpression(parent)) {
            const parameterPosition = argFunction.getParameters().findIndex(p => p.getName() === argName);
            asserts(parameterPosition > -1, `No argument named '${ argName }!`);
            locateCallExpressionQuery(parent, ref.getText(), parameterPosition, queries, item, filename);
          }
        ...
        if (Node.isPropertyAccessExpression(callee)) {
          if (callee.getName() === 'map') {
            ...
            asserts(Node.isIdentifier(mapExpression), `Unexpected expression kind: '${ mapExpression.getKindName() }'!`);
            asserts(mapExpression.getType().isArray(), `Unexpected expression type: '${ mapExpression.getKindName() }'!`);
    ...
    asserts(Node.isIdentifier(propertyIdentifier), `Expected an identifier ${ propertyIdentifier.getText() }`);
    for (const ref of propertyIdentifier.findReferencesAsNodes()) {
      ...
      if (Node.isJsxAttribute(parentRef)) {
        ...
        asserts(Node.isJsxExpression(initializer), `JSX attribute value expected to be a JSX expression, but was '${ initializer?.getKindName() }'!`);
        asserts(initializer.getChildCount() === 3, `Expected JSX expression matching the shape [Node, Identifier, Node]!`);

        const variableIdentifier = initializer.getChildAtIndex(1);
        asserts(Node.isIdentifier(variableIdentifier), `Expected the JSX expression having an identifier in the middle place, found '${ variableIdentifier.getKindName() }'!`);
        locateIdentifierQuery(variableIdentifier, variableIdentifier.getText(), queries, item, filename);
      }
      else if (Node.isPropertyAssignment(parentRef)) {
        const initializer = parentRef.getInitializer();
        asserts(Node.isIdentifier(initializer), `Unexpected property assignment initializer kind: '${ initializer!.getKindName() }'`);
        locateIdentifierQuery(initializer, initializer.getText(), queries, item, filename);
      }
      else if (!Node.isPropertySignature(parentRef) && !Node.isCallExpression(parentRef) && !Node.isBinaryExpression(parentRef) && !Node.isPropertyAccessExpression(parentRef)) {
